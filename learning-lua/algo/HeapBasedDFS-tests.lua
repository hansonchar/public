local HeapBasedDFS = require "algo.HeapBasedDFS"
...
local function tim_test()
  print("HeapBasedDFS tim_test...")
  local input<const> = {'s-v=1', 's-w=4', 'v-w=2', 'v-t=6', 'w-t=3'}
  local src<const> = 's'
  local G = load_input(input)
  local level_counts = {}
  for from, to, weight, level, order in HeapBasedDFS:new(G, src):iterate() do
